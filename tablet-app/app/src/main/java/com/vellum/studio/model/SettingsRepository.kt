...
class SettingsRepository(context: Context) {
    ...
    private val debugFrameOverlayEnabledState = mutableStateOf(prefs.getBoolean(KEY_DEBUG_FRAME_OVERLAY_ENABLED, false))

    /**
     * Debug-only on-canvas HUD (see [com.vellum.studio.canvas.DrawingCanvasView]'s frame-time
     * overlay) showing recent frame times and a 120Hz dropped-frame count -- instrumentation for a
     * human tester benchmarking real S Pen stroke smoothness on this panel, not a user-facing
     * feature. Off by default, same as the other Experimental toggles.
     */
    var debugFrameOverlayEnabled: Boolean
        get() = debugFrameOverlayEnabledState.value
        set(value) {
            debugFrameOverlayEnabledState.value = value
            prefs.edit().putBoolean(KEY_DEBUG_FRAME_OVERLAY_ENABLED, value).apply()
        }
    ...
    private companion object {
        ...
        const val KEY_DEBUG_FRAME_OVERLAY_ENABLED = "debug_frame_overlay_enabled"
    }
}
